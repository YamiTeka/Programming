...
void yandex_hook(const Request& req, Response& res)
{
	...
	if ((*cur_session)["skill_mode"] == help_mode)
	{
		...
		else if (command == "корзина")
		{
			text = "Я запомню, что вы будете добавлять или удалять из корзины.\n"
				"Чтобы добавить что-то, скажите \"Добавить в корзину сыр 120 рублей\".\n"
	...
	else
	{
			...
			for (auto entity : req_json["request"]["nlu"]["entities"])
			{
				if (entity["type"].get<std::string>() == "YANDEX.NUMBER")
				{
					...
					//{
						...
						{
							text = "Бесплатный сыр только в мышеловке";
							tts = "Беспл+атный сыр толь+ко в мыш+еловке";
						}
						else
						{
							item_price = val;
			  ...
			  || command.find("убрать из корзины") == 0  || command.find("убери из корзины") == 0)
		{
			...
			if (item_name == "")
			{
				text = "Что собираетесь убирать?";
				tts = "тЧо собир+аетесь убир+ать?";
			}
			else
			{
				item_name.pop_back();
				...
				if (!found_item)
				{
					...
					text = "Такого в вашей корзине нет";
					tts = "Так+ого в в+ашей корз+ине н+ет.";
				}
				else
				{
